...
class VaultSnapshot {
  ...
  static VaultSnapshot decode(String raw) {
    ...
    final accounts = _mapList(record['accounts'], Account.fromJson)
        .where((account) => account.id.isNotEmpty && account.baseUrl.trim().isNotEmpty)
        .toList();
    final accountIds = accounts.map((account) => account.id).toSet();
    final apiKeys = _mapList(record['apiKeys'], ApiKey.fromJson)
        .where((apiKey) => accountIds.contains(apiKey.accountId) && apiKey.id.isNotEmpty)
        .toList();
    ...
    return VaultSnapshot(
      ...
      checkinLogs: _mapList(record['checkinLogs'], CheckinLog.fromJson)
          .where((log) => accountIds.contains(log.accountId))
          .toList(),
      usageLogs: _mapList(record['usageLogs'], UsageLog.fromJson)
          .where((log) => accountIds.contains(log.accountId))
          .toList(),
  ...
  static VaultSnapshot merge(VaultSnapshot current, VaultSnapshot incoming) {
    ...
    revealed.removeWhere((key, value) => !apiKeyIds.contains(key) || value.isEmpty);
    final passwords = Map<String, String>.from(current.accountPasswords)
      ..addAll(incoming.accountPasswords);
    passwords.removeWhere((key, value) => !accountIds.contains(key) || value.isEmpty);
    return VaultSnapshot(
...
BackupPeek peekBackup(String raw) {
  ...
  if (record['encrypted'] == true) {
    ...
    return BackupPeek(
      encrypted: true,
      exportedAt: DateTime.tryParse(record['exportedAt']?.toString() ?? '')?.toUtc(),
